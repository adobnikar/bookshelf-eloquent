...
exports.seed = async function(knex, Promise) {
  ...
  let posts = (await Post.select('postIdAttr').withTrashed().get()).toJSON();
  ...
  while (commentsCount > 0) {
    ...
    commentsCollection.add({
      ...
      createdById: (rng.int(5) === 1) ? null : user.userIdAttr, // A random chance this comment was created by an anonymous user.
